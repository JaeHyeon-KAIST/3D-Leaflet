(() => {
  ...
  function closeLeaflet() {
    pageCount = 0;
    document.body.classList.remove("leaflet-oppend");
  ...
  function zoomIn(elem) {
    ...
    const dy = window.innerWidth / 2 - (rect.y + rect.height / 2);
    let angle;
  ...
  leaflet.addEventListener("click", (e) => {
    ...
    if (pageElem) {
      ...
      if (pageCount == 2) {
        document.body.classList.add("leaflet-oppend");
      }
    }
    ...
    if (menuItemElem && !document.body.classList.contains("zoom-in")) {
      zoomIn(menuItemElem);
    }
